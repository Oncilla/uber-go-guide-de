...
- [Richtlinien](#richtlinien)
  - [Pointers auf Interfaces](#pointers-auf-interfaces)
...
Styles sind die Konventionen die unseren code bestimmen. Der Begriff Style ist
jedoch ein leichter Misnomer, da diese Konventionen bei weitem mehr umfassen
als die Formatierun von Soure Datein - gofmt regelt das bereits für uns.

Das Ziel dieses Guides ist es diese Komplexität zu bewältigen indem er die Dos
und Don'ts der Go Entwicklung bei Uber aufzeigt. Diese Regeln existieren um die
Code Base handlich zu halten, jedoch erlauben sie auch den Engineers die
Go Language Features produktive zu nutzen.

Dieser Guide wurde ursprünglich von [Prashant Varanasi] und [Simon Newton] ins
Leben gerufen um Mitarbeitended in Sachen Go auf Trab zu bringen. Im laufe der
Jahre wurde er durch Feedback der Community überarbeited.

  [Prashant Varanasi]: https://github.com/prashantv
  [Simon Newton]: https://github.com/nomis52

Hier werden die idiomatischen Konventionen für Go Code beschrieben den wir
bei Uber befolgen. Viele dieser Konventionen sind generelle Guidelines für Go.
Einige bauen of externen Resourcen auf:

1. [Effective Go](https://golang.org/doc/effective_go.html)
2. [The Go common mistakes guide](https://github.com/golang/go/wiki/CodeReviewComments)

Der ganze Code muss fehlerfrei durch `golint` und `go vet` laufen. Wir empfehlen,
dass du dein Editor wiefolgt einstellst:

- Führe `goimports` bei jedem Speichern aus.
- Führe `golint` und `go vet` aus um Fehler zu finden.

Du kannst weitere Informationen zur Editor Unterstützung von Go Tools hier
finden:
...
### Pointers auf Interfaces

Du solltest fast nie Pointer auf Interfaces benötigen. Du solltest Interfaces
als Value an Funktionen übergeben - Die darunterliegende Daten können immernoch
ein Pointer sein.

Ein Interface besteht aus zewi Feldern:

1. Ein Pointer auf ein Type-spezifische Information. Du kannst es als "type"
   ansehen.
2. Data Pointer. Falls die Data bereits ein Pointer ist, wird er direkt in
   diesem Feld gespeichert. Falls die Data ein Value ist, wird ein Pointer
   zu dem Value in diesem Feld gespeichert.

Falls du willst das Interface methoden die darunnterliegendedn Daten
modifizieren, musst du einene Pointer verwenden.

### Verifiziere Interface Konformität

Verifiziere Interface Konformität zur Kompilierungszeit wo angebracht.
Einschliesslich:

- Exportierte Typen die als Teil ihres API Vertrags ein spezifisches Interface
  implementieren müssen.
- Exportierte oder unexportierte Typen die mit anderen Typen das selbe Interface
  implementieren.
- Andere Fälle wo die Verletzung des API Vertrags zu Problemen bei Usern führt.
...
Das Statement `var _ http.Handler = (*Handler)(nil)` scheitert zu kompilieren
sobald `*Handler` das `http.Handler` Interface nicht mehr erfüllt.

Die rechte Seite des Assignemnts sollte der Zero-Value des asserteten Types
sein. Für Pointers, Slices und Maps ist dies `nil` (z.B. `*Handler`). Für
Struct Types ist dies das Empty Struct.
...
### Receivers und Interfaces

Methoden mit Value Receivern können auf Pointern und Values aufgerufen werden.
Methoden mit Pointer Receivern können nur auf Pointern oder [addressierbaren
Values] aufgerufen werden.

  [addressierbaren Values]: https://golang.org/ref/spec#Method_values
...
Analog kann ein Interface durch ein Pointer erfüllt werden, sogar wenn die
Methode einen Value Receiver hat.
...
Effective Go hat eine gute Zusammenstellung von [Pointers vs. Values].

  [Pointers vs. Values]: https://golang.org/doc/effective_go.html#pointers_vs_values

### Zero-Value Mutexes sind valid

Der Zero-Value von `sync.Mutex` und `sync.RWMutex` sind valide. Du benötigst
fast nie einen Pointer zu einem Mutex.
...
Wenn du ein ein Struct per Pointer benutzts, kann das Mutex ein non-Pointer Feld
sein.

Unexported Structs die ein Mutex benutzen um Felder im Struct zu schützen,
können das Mutex embedden.
...
<tr>
<td>Embedde für unexportierte typen, oder typen die das Mutex interface implementieren müssen</td>
<td>Benutze ein privates Feld für exportierte Typen</td>
</tr>

</tbody></table>

### Kopiere Slices und Maps an Grenzen

Slices und Maps beinhalten einen Pointer zu den darunterliegenden Daten.
Desshalb, halte ausschau nach Szenarien wo die Daten kopiert werden müssen.


#### Slices and Maps entgegen nehmen
